...
def test_safe_attribute_value_serializes_structured_values():
    ...
    assert safe_attribute_value(None) is None
    assert safe_attribute_value({"nested": {"a": 1}}) == '{"nested":{"a":1}}'
    assert safe_attribute_value(["a", 1]) == '["a",1]'
    assert safe_attribute_value({"bad": _Unserializable()}) == "{'bad': unserializable}"
